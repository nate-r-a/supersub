...
class SuperSubTest < Minitest::Test
  ...
  def test_superscript
    %i[superscript super].each do |keyword|
      assert_equal "ᵃᵇᶜᵈᵉᶠᵍʰⁱʲᵏˡᵐⁿᵒᵖqʳˢᵗᵘᵛʷˣʸᶻᴬᴮᶜᴰᴱᶠᴳᴴᴵᴶᴷᴸᴹᴺᴼᴾQᴿˢᵀᵁⱽᵂˣʸᶻ⁰¹²³⁴⁵⁶⁷⁸⁹.,?!@\#$%^&*()[]{}<>/\\'\"-_=+`~",
        SuperSub.convert(@string, keyword)
    end
  end
  ...
  def test_script
    %i[script].each do |keyword|
      assert_equal "𝒶𝒷𝒸𝒹ℯ𝒻ℊ𝒽𝒾𝒿𝓀𝓁𝓂𝓃ℴ𝓅𝓆𝓇𝓈𝓉𝓊𝓋𝓌𝓍𝓎𝓏𝒜ℬ𝒞𝒟ℰℱ𝒢ℋℐ𝒥𝒦ℒℳ𝒩𝒪𝒫𝒬ℛ𝒮𝒯𝒰𝒱𝒲𝒳𝒴𝒵0123456789.,?!@\#$%^&*()[]{}<>/\\'\"-_=+`~",
        SuperSub.convert(@string, keyword)
    end
  end
  ...
  def test_small_caps
    %i[small_caps small].each do |keyword|
      assert_equal "ᴀʙᴄᴅᴇꜰɢʜɪᴊᴋʟᴍɴᴏᴘqʀꜱᴛᴜᴠᴡxʏᴢᴀʙᴄᴅᴇꜰɢʜɪᴊᴋʟᴍɴᴏᴘqʀꜱᴛᴜᴠᴡxʏᴢ0123456789.,?!@\#$%^&*()[]{}<>/\\'\"-_=+`~",
        SuperSub.convert(@string, keyword)
    end
  end
